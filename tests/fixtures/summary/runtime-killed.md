...
**This scan is incomplete — checkov did not finish.** Anything below is partial, and a nil result would not be evidence.
...
> exploiting.
> **This is generation `00000000-0000-4000-8000-000000000006`.** Every JSON file in this folder carries the same `generation`; one that does not is from another run.

## ⚠ Scanners that did not complete

- **checkov** — exit 137: killed by the runtime — the container's memory ceiling (2g) or the VM's
...
_Scanners ran concurrently; slowest: checkov 48.3s. Each one's time is in `run.json`._
