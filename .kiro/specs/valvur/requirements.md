...
4. F10.4 — **CORRECTED 2026-08-30.** ~~The image SHALL contain no GPL- or
   ...
   *Extended 2026-09-21 (task 27.0.2): "the SBOM" is one per architecture the
   image ships for, each checked to describe the child it names, generated by
   the syft the image itself pins by digest — until then the published SBOM
   described amd64 only and was made by a syft named by tag.*
